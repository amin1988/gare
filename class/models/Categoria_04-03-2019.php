...
class Categoria extends Modello {
	...
	public static function parse($codice) {
		if ($codice[0] != 'W') return NULL;
		if (strlen($codice) != 15) return NULL;
		$c = new Categoria();
		...
		$grado = hexdec($grado);
		if ($grado <= 0 || $grado >= 128) return NULL;
		$g=array();
		if (($grado & 1) != 0)
			$g[] =1;
		if (($grado & 2) != 0)
		...
		if ($c->getTipo() == 0)
		{
			...
			if ($stile <= 0 || $stile >= 16) return NULL;
		...
		if ($c->getTipo() == 1 || $c->getTipo() == 2)
		{
			...
			if ($pesomax >= 254) $pesomax = self::MAX_PESO;
	...
	public function getCinture() {
		if (!is_null($this->cinture)) return $this->cinture;
	...
	public function getStili() {
		if (!is_null($this->stili)) return $this->stili;
...
// 	}
	...
	public function getNome($rigenera=false) {
		if (!$rigenera && !is_null($this->nome)) return $this->nome;
		/*
		if($this->getGruppo() == 33)
			return $this->getNomeEta();
		*/
		$tipo = $this->getTipo();
		$ind = GruppoCat::fromId($this->getGruppo())->isIndividuale();
		
		if($this->getGruppo() == 29)
		...
		$cin = $this->getCinture();
		if (count($cin) == 1) $cinstr = Cintura::getCintura(current($cin))->getNome();
		else {
	...
	/**
	 * indica se ad una certa data l'atleta può appartenere alla categoria
	 * @access public
	 * @param Atleta $atleta 
	 * @param Data $data
	...
	/**
	 * indica se ad una certa data un iscritto individuale può appartenere alla categoria
	 * @access public
	...
	public function individualeInCategoria($atleta, $iscr, $data) {
		...
		//stile
		if (count($this->getStili()) > 0 
				&& !in_array($iscr->getStile(), $this->getStili()))
			return false;
		return true;
	}
	...
	public function squadraInCategoria($sesso, $etamin, $etamax, $tipo, $cinture) {
		...
		if (is_null($ecm)) $ecm = $this->getEtaMin();
		if ($ecm > $etamin)
			return false;
		...
		//cerca la cintura più alta
		$max = NULL;
		foreach ($cinture as $idc) {
			if ($max === NULL || $idc > $max)
				$max = $idc;
		}
		//la citura più alta dev'essere in questa categoria
		if (!in_array($max, $this->getCinture()))
			return false;
		
		if($gruppo == 46 || $gruppo == 47 || $gruppo == 48)
		{
			//in questi gruppi c'è sovrapposizione di cinture
			//identifico la categoria giusta dalla cintura più bassa
			//cerca cintura più bassa
			$min = NULL;
			foreach ($cinture as $idc) {
				if ($min === NULL || $idc < $min)
					$min = $idc;
			}
			//la citura più bassa dev'essere in questa categoria
	...
	/**
	 * Indica se la categoria è di agonisti
	 * @return boolean
	 */
	...
	public function uguale($cat) {
		if ($this->getEtaMax() != $cat->getEtaMax()) return false;
		if ($this->getTipo() != $cat->getTipo()) return false;
		if ($this->getSesso() != $cat->getSesso()) return false;
		if ($this->getEtaMin() != $cat->getEtaMin()) return false;
		if ($this->getPesoMax() != $cat->getPesoMax()) return false;
		if ($this->getPesoMin() != $cat->getPesoMin()) return false;
		//cinture
		if (!$this->insiemeUguale($this->getCinture(), $cat->getCinture())) return false;
		//stili
		if (!$this->insiemeUguale($this->getStili(), $cat->getStili())) return false;
		return true;
	}
	
	private function insiemeUguale($arr1, $arr2) {
		if (is_null($arr1)) {
			if (is_null($arr2)) return true; //entrambi null
			else return false; //solo 1 null
		}
		if (is_null($arr2)) return false; //solo 2 null
		if (count($arr1) != count($arr2)) return false;
		foreach ($arr1 as $e) {
			if (!in_array($e, $arr2)) return false;
		}
		return true;
	}
...
// 	 * 0 per caricare solo quelle già esistenti
