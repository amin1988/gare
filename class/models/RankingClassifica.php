...
class RankingClassifica extends Modello {
        ...
        private function getRowClassificaRanking($categoria, $dati_parametri_ricerca) {
	     ...
	     while ($row = $result->fetch_assoc()) {
		  ...
		  $row['punteggio_ranking'] = $row['punteggio_ranking'] ;
        ...
        public function componiClassificaRanking($dati_post, $tipo_ev) {
                /*
            ...
            FROM ranking_risultati as r INNER JOIN gare AS g ON r.id_gara = g.idgara 
             WHERE YEAR(g.data) = " . $anno_selezionato . "  AND r.id_gara=$id_gara";
	     ...
	     if ($res) {
	             while ($row = $res->fetch_assoc()) {
		  $id_gara_stage = $row['id_gara'];

		  $trovato = $this->controllaInRankingClassifica($id_gara_stage);
		  if ($trovato) {
		          continue;
		  }
		  $gara = new gara($id_gara_stage);
		  $tipologia_evento = $gara->getTipoGara();
		  $this->setTipologiaGara($tipologia_evento);
		  $risultati_gara = $this->getAllRisultatiGara($id_gara_stage);
		  $this->calcola_punteggio($risultati_gara, $id_gara_stage);
		  $this->insertResult($anno_selezionato, $id_gara_stage);
	             }
	     }
                }
        }
        ...
        public function getAllRisultatiGara($id_gara) {
	     ...
	     . " WHERE id_gara = " . $id_gara . " AND classificato<7  ";
                $res = $this->conn->query($sql);
